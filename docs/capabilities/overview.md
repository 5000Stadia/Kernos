...
> **Looking for the full kernel-tool list?** The kernel provides 76 tools that ship with Kernos itself (memory, files, references, canvas, covenants, channels, scheduling, diagnostics, coding sessions, git, the self-improvement loop, etc.). See [`tool-surface.md`](tool-surface.md) for the complete catalog with effect classifications and per-tool documentation links.
