...
**What we built:** The Reasoning Service (1B.2) already abstracts the LLM call. Today it calls gpt-5.6-sol (with gpt-5.4-mini on the lightweight tier) via the Codex provider. The interface is: "here's a reasoning request, give me a response." Swapping in model routing later means changing the Reasoning Service internals, not the agent interface.
