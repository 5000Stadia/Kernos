...
ASK_CODING_SESSION_TOOL: dict = {
    ...
    "description": (
        "Decision rule: can keep working on something else while "
        "the external agent investigates → `ask_coding_session` "
        "(non-blocking). Need the answer before your next step → "
        "`consult` (blocks in-turn). Same external CLIs (Claude "
        "Code, Codex), same ACPX substrate; only the blocking "
        "model differs.\n\n"
        "Submits to the outbound bridge; the external agent is "
        "dispatched through ACPX (Agent Client Protocol) to a "
        "fresh CLI invocation. Returns a `request_id` immediately. "
        "Poll via `read_coding_session_response(request_id)` — it "
        "returns 'attempted' (STILL IN FLIGHT, safe to poll again "
        "later — not a failure), 'completed' with the response, or "
        "'failed' on timeout.\n\n"
        "Filesystem effects: the external agent has read AND WRITE "
        "access to the repo by default — file changes it makes "
        "persist. Treat as effects in the world. Ask for "
        "advisory-only explicitly if that's what you want.\n\n"
        "When to use: verify a code claim, audit substrate "
        "behavior, get a second opinion, kick off an investigation "
        "you can parallelize with other work. The structured "
        "`context` (suspected_paths, related_conversation, "
        "prior_decisions) scopes the agent's investigation — use "
        "it; vague questions get vague answers. When NOT to use: "
        "your very next decision depends on the result (use "
        "`consult`); the task is tiny enough that waiting is "
        "cheaper than polling; you can answer it yourself with a "
        "grep or two."
    ),
    "input_schema": {
        "type": "object",
