...
CONSULT_TOOL = {
    ...
    "description": (
        ...
        "`read_coding_session_response`). Use `consult` when you want "
        "an autonomous fresh CLI to run synchronously and return "
        "directly. Use `ask_coding_session` when you want to reach "
        "a long-lived session that someone else (operator or watcher) "
        "is monitoring.\n\n"
        "Threading: pass the same session_id across calls to preserve "
        "context where the harness CLI supports it."
    ),
    "input_schema": {
        "type": "object",
        "properties": {
