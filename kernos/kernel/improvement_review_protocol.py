...
_ROLE_FRAMING = {
    ...
    "spec_reviewer": (
        ...
        "REQUEST FIDELITY: the spec must preserve EVERY concrete "
        "deliverable the operator named (specific function names, "
        "files, behaviors, values). If the spec dropped, renamed, "
        "or substituted one of the operator's named deliverables "
        "for a different/easier one, that is NEEDS_REVISION — name "
        "the operator deliverable that's missing from the spec. "
        "(The spec author can be drafting from a stale/garbled "
        "spec.md; your job is to catch drift from the real ask.)"
    ),
    "impl_author": (
    ...
    "impl_reviewer": (
        ...
        "REQUEST-FIDELITY GATE (this is the final check before the "
        "operator's approval — it is your most important job). The "
        "'Operator's improvement requirement' below is the literal "
        "ask. The spec.md is only an internal relay and CAN drift "
        "from it. Before GREEN, confirm the diff actually fulfils "
        "the OPERATOR'S request — every concrete deliverable they "
        "named (a specific function name, file, behavior, or value) "
        "must be present and correct in the diff. If the spec or the "
        "implementation SUBSTITUTED, renamed, dropped, or simplified "
        "a deliverable the operator explicitly named — e.g. they "
        "asked for `humanize_duration` and the diff ships a different "
        "helper — that is NEEDS_REVISION even when the code is "
        "correct, tested, and self-consistent. Correct work on the "
        "wrong deliverable is a FAILURE. (Latitude on HOW it's built "
        "is fine; substituting WHAT was asked for is not.) When you "
        "send NEEDS_REVISION for a fidelity gap, name the operator's "
        "deliverable that's missing and what shipped instead."
    ),
}
